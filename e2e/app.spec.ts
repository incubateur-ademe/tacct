...
test.describe('Homepage', () => {
  test('should load and display main content', async ({ page }: any) => {
    await page.goto('/');
...
test.describe('Recherche territoire', () => {
  test('should load the search page', async ({ page }: any) => {
    await page.goto('/recherche-territoire');
    await expect(page).toHaveURL(/recherche-territoire/);
  });

  test('should load the patch4 search page', async ({ page }: any) => {
...
test.describe('Données territoire', () => {
  test('should load donnees page for PNR de Camargue', async ({
    page
  }: any) => {
    await page.goto(
  ...
  test('should load patch4c page for PNR de Camargue', async ({
    page
  }: any) => {
    await page.goto(
...
test.describe('Ressources', () => {
  test('should load ressources index', async ({ page }: any) => {
    await page.goto('/ressources');
    await expect(page).toHaveURL(/ressources/);
    await expect(page.locator('main h1').first()).toBeAttached();
  });

  test('should load demarrer-diagnostic-vulnerabilite', async ({
    page
  }: any) => {
  ...
  test('should load analyser-diagnostic-vulnerabilite', async ({
    page
  }: any) => {
    await page.goto(
...
test.describe('Pages statiques', () => {
  test('accessibilite should load', async ({ page }: any) => {
    await page.goto('/accessibilite');
    await expect(page).toHaveURL(/accessibilite/);
    await expect(page.locator('main h1').first()).toBeVisible();
  });

  test('mentions-legales should load', async ({ page }: any) => {
    await page.goto('/mentions-legales');
    await expect(page).toHaveURL(/mentions-legales/);
    await expect(page.locator('main h1').first()).toBeVisible();
  });

  test('budget should load', async ({ page }: any) => {
...
test.describe('Erreurs', () => {
  test('should show 404 page for unknown routes', async ({ page }: any) => {
