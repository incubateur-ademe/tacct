...
import {
  sommaireImpacts,
  sommaireThematiques
} from '@/app/(main)/(parcours)/thematiques/constantes/textesThematiques';
...
import {
  handleRedirection,
  handleRedirectionThematique
} from '@/hooks/Redirections';
...
export const MenuLateral = ({
  ...
  const thematique = searchParams.get('thematique') as
    | 'Confort thermique'
    | 'Gestion des risques'
    | 'Aménagement'
    | 'Eau'
  ...
  const [openEtape1, setOpenEtape1] = useState<boolean>(
    params === '/donnees' ? true : false
  );
  const [openEtape2, setOpenEtape2] = useState<boolean>(
    params === '/impacts' ? true : false
  );
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const screenHeightAboveFooter =
        windowHeight + (distanceFromBottom - footerHeight);
      if (
      ...
      if (params === '/donnees') {
        const allAnchors = ongletsMenuEtape1.thematiquesLiees.flatMap(
          (section) => section.sousCategories
        );
        ...
        >
          ...
          {isContentVisible && (
            <>
              <div>
                {/* Bouton de retour */}
                <div style={{ borderBottom: '1px solid var(--gris-medium)' }}>
                  <a
              ...
              <div className="flex flex-col" ref={navigationRef}>
                ...
                >
                  ...
                  <Body
                    ...
                    htmlTag="span"
                    style={{
                      color: openEtape1 ? 'var(--principales-vert)' : 'black'
                    }}
                  >
                ...
                <div
                  className={
                    thematique === 'Confort thermique'
                      ? styles.menuEtapeDonnees
                      : styles.menuEtapeDonneesSansImpact
                  }
                >
                  {openEtape1 &&
                    ongletsMenuEtape1?.thematiquesLiees.map(
                      (thematique, id) => (
                            ...
                            {thematique.sousCategories.map((item) => (
                              <li key={item}>
                                <button
                                  ...
                                  aria-current={
                                    activeAnchorEtape1 === toAnchorId(item)
                                      ? 'location'
                                      : undefined
                                  }
                      ...
                      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        <li>
                          <button
                            type="button"
                            onClick={() =>
                              handleItemClickEtape1('Érosion côtière')
                            }
                            aria-current={
                              activeAnchorEtape1 === 'Érosion-côtière'
                                ? 'location'
                                : undefined
                            }
                            className={`block w-full text-left p-2 text-sm rounded-md transition-colors ${
                  ...
                  <>
                    ...
                    >
                      ...
                      <Body
                        ...
                        style={{
                          color: openEtape2
                            ? 'var(--principales-vert)'
                            : 'black'
                        }}
                      >
                        Étape 2. <br />
                        Diagnostiquez les impacts
                      </Body>
                    </button>
                    <ul
                      className={styles.menuEtapeImpacts}
                      style={{ listStyle: 'none', padding: 0, margin: 0 }}
                    >
                      {openEtape2 &&
                        ongletsMenuEtape2.map((item) => (
                          <li key={item.id}>
                            <button
                              type="button"
                              onClick={() => handleItemClickEtape2(item)}
                              aria-current={
                                activeAnchorEtape2 === item.id
                                  ? 'location'
                                  : undefined
                              }
