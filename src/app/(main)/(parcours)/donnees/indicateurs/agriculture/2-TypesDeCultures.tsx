...
export const TypesDeCulture = (props: {
  ...
  const otexCommune =
    type === 'commune' &&
    tableCommune.find((el) => el.code_geographique === code)?.otex_12_postes;
          ...
          <div className={styles.chiffreDynamiqueWrapper}>
            ...
            {surfacesAgricoles.length ? (
              <>
                ...
                maxCategory.count !== null &&
                Number(sau) !== 0 ? (
                  <>
                    <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                      Sur votre territoire, le type de surface prédominant est
                      constitué de {maxCategory.id.toLowerCase()}, couvrant{' '}
                      <b>{numberWithSpacesRegex(maxCategory.count)} hectares</b>
                      , ce qui représente{' '}
                      <b>{Round((maxCategory.count / sau!) * 100, 1)} %</b> de
                  ...
                  <>
                    <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                      Sur votre commune, c'est le/la "{otexCommune}" qui domine
                      le paysage agricole avec plus des deux tiers de la
                      production totale{' '}
                      <DefinitionTooltip title={otex}>(OTEX)</DefinitionTooltip>
                      .
                    </Body>
                    <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                      Sur votre territoire, le type de surface prédominant est
                      ...
                      , ce qui représente{' '}
                      <b>{Round((maxCategory.count / sau!) * 100, 1)} %</b> de
                      la surface agricole utile. (Attention, ces détails sur les
                      types de cultures sont ceux de votre EPCI).
                    </Body>
                  </>
                ) : (type === 'epci' || type === 'petr') &&
                  maxCategory.count !== null &&
                  ...
                  <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                    ...
                    <b>{numberWithSpacesRegex(maxCategory.count)} hectares</b>,
                    ce qui représente{' '}
                    <b>{Round((maxCategory.count / sau!) * 100, 1)} %</b> de la
