...
const MapSurfacesIrriguees = lazy(() =>
  import('@/components/maps/mapSurfacesIrriguees').then((m) => ({
    default: m.MapSurfacesIrriguees
  }))
);
...
export const SuperficiesIrriguees = (props: {
  ...
  const surfaceTerritoire =
    type === 'commune'
      ? tableFiltered[0]?.part_irr_sau_2020
        ? Number(tableFiltered[0].part_irr_sau_2020)
        : undefined
  ...
  const exportData =
    IndicatorExportTransformations.agriculture.surfacesIrriguees(tableFiltered);
  ...
  return (
    <>
            ...
            <>
              ...
              <div className={styles.text}>
                <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                  Une réalité locale : {Round(surfaceTerritoire ?? 0, 1)} % de
      ...
      {surfaceTerritoire !== undefined &&
      !isNaN(surfaceTerritoire) &&
      surfaceTerritoire !== 0 ? (
        ...
        >
          ...
          <Body size="sm" style={{ color: 'var(--gris-dark)' }}>
            Export indisponible : données non référencées ou nulles.
          </Body>
        </div>
      )}
    </>
