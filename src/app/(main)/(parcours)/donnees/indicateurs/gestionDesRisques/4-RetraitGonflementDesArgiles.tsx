...
export const RetraitGonflementDesArgiles = ({
  ...
  const [datavizTab, setDatavizTab] = useState<string>(
    type === 'commune' || type === 'epci' ? 'Comparaison' : 'Répartition'
  );
  const rgaFilteredByTerritory =
    type === 'commune'
  ...
  const partMoyenFort =
    rgaFilteredByTerritory.length > 0
      ? Average(
          rgaFilteredByTerritory.map((el) => el.part_alea_moyen_fort_commune)
        )
      : 0;
  const nbLogementsMoyenFort =
    rgaFilteredByTerritory.length > 0
      ? Sum(rgaFilteredByTerritory.map((el) => el.nb_logement_alea_moyen_fort))
      : 0;
  const partMoyenFortApres1975 =
    rgaFilteredByTerritory.length > 0
      ? Round(
          (100 *
            Sum(
            ...
            Sum(
              rgaFilteredByTerritory.map((el) => el.nb_logement_alea_moyen_fort)
            ),
          1
        )
      : 0;

  const exportData = IndicatorExportTransformations.gestionRisques.RGA(
    rgaFilteredByTerritory
  );
      ...
      <div className={styles.datavizContainer}>
        <div className={styles.dataTextWrapper}>
          <div className={styles.chiffreDynamiqueWrapper}>
            {rgaFilteredByTerritory.length > 0 && (
              <MicroCircleGrid
                pourcentage={partMoyenFort}
                arrondi={1}
                ariaLabel="Part des logements avec une exposition moyenne ou forte"
              />
            )}
            {rgaFilteredByTerritory.length > 0 ? (
              <>
                <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                  {Round(partMoyenFort, 1)} % de votre territoire est situé dans
                  une zone où le niveau d’exposition au retrait gonflement des
                  argiles est moyen ou fort. Cela concerne potentiellement{' '}
                  {numberWithSpacesRegex(nbLogementsMoyenFort)} logement(s),
                  parmi lesquels{' '}
                  <b>
                    {nbLogementsMoyenFort === 0 ? 0 : partMoyenFortApres1975} %
                  </b>{' '}
                  sont considérés comme plus à risque car construits après 1975.
                </Body>
                <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                  ⚠️ Ces chiffres reposent sur l'ancien zonage RGA. La
                  cartographie actualisée du BRGM (début 2026) reflète une
