...
export const GrandAge75 = ({
  ...
  const libelleDepartement =
    type === 'epci' ? confortThermique[0]?.libelle_departement : '';
  ...
  const grandAgeMapped = confortThermique.map(grandAgeMapper);
  const territoireSup =
    type === 'epci'
      ? grandAgeMapped.filter((e) => e.departement === departement)
      : grandAgeMapped;
  ...
  const exportData =
    IndicatorExportTransformations.inconfort_thermique.GrandAge75(
      grandAgeTerritoire
    );
      ...
      <div className={styles.datavizContainer}>
        <div className={styles.dataTextWrapper}>
          <div className={styles.chiffreDynamiqueWrapper}>
            <MicroCircleGrid
              pourcentage={Number(yData.over_75_2022_percent)}
              arrondi={2}
              ariaLabel="Pourcentage des personnes âgées de plus de 80 ans"
            />
            {!Object.values(yData).slice(0, -2).includes('NaN') && (
              <>
                <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                  En 2022,{' '}
                  <b>{numberWithSpacesRegex(yData.over_75_2022_percent)} %</b>{' '}
                  de la population de votre territoire est constitué de
                  personnes âgées de plus de 75 ans (soit{' '}
                  <b>
                    {numberWithSpacesRegex(
            ...
            {type === 'commune' &&
            percentTerritoireSup &&
            percentTerritoireSup !== 'NaN' ? (
              <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
              ...
              percentTerritoireSup !== 'NaN' ? (
              <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                Ce taux est de {percentTerritoireSup} % dans ce département :{' '}
                {libelleDepartement}.
              </Body>
            ) : (
              ''
          ...
          {yData.over_75_2022_percent ? (
            <div
              style={{
                ...
                height: !Object.values(yData).slice(0, -2).includes('NaN')
                  ? '500px'
                  : 'max-content',
                width: '100%',
              ...
              {!Object.values(yData).slice(0, -2).includes('NaN') ? (
                <>
                  <LineChartGrandAge
                    xData={xData}
                    yData={yGraphData}
                    disclaimer={multipleDepartements.length > 1}
                  />
                    ...
                    >
                      <div className="flex flex-row items-center justify-center">
                        ...
                        <Body style={{ fontSize: 12 }}>
                          L'EPCI sélectionné s'étend sur plusieurs départements.
                          La comparaison proposée est effectuée avec :{' '}
                          {libelleDepartement}
                        </Body>
                      </div>
                    </div>
