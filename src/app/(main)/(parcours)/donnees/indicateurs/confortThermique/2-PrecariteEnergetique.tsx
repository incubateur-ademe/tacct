...
const MapConfortThermique = lazy(() =>
  import('@/components/maps/mapConfortThermique').then((m) => ({
    default: m.MapConfortThermique
  }))
);
...
export const PrecariteEnergetique = ({
  ...
  const confortThermiqueFiltered = confortThermique.filter(
    (e) => e.precarite_logement !== null && !isNaN(e.precarite_logement)
  );
  const confortThermiqueParMaille =
    type === 'epci'
      ? confortThermiqueFiltered.filter((obj) => obj.epci === code)
      : type === 'commune'
        ? confortThermiqueFiltered.filter(
            (obj) => obj.code_geographique === code
          )
        : type === 'petr'
          ? confortThermiqueFiltered.filter(
              (obj) => obj.libelle_petr === libelle
            )
          : type === 'ept'
            ? confortThermiqueFiltered.filter((obj) => obj.ept === libelle)
            : type === 'pnr'
              ? confortThermiqueFiltered.filter(
                  (obj) => obj.libelle_pnr === libelle
                )
    ...
    <>
            ...
            {precariteLogTerritoire ? (
              <>
                <Body weight="bold" style={{ color: 'var(--gris-dark)' }}>
                  La précarité énergétique va au-delà du seul critère du revenu,
                  elle inclut les mauvaises conditions d’habitation ainsi que
                  les évolutions du prix des énergies. La part des ménages en
                  situation de précarité énergétique liée au logement sur votre
                  territoire est de{' '}
                ...
                {type === 'commune' && (
                  <Body
                    weight="bold"
                    style={{ color: 'var(--gris-dark)', padding: '0.5rem 0' }}
                  >
                    Ce taux est de {Round(100 * precariteLogTerritoireSup, 1)} %
                    dans votre EPCI.
                  </Body>
                )}
      ...
      <div className={styles.sourcesExportMapWrapper}>
        <Body size="sm" style={{ color: 'var(--gris-dark)' }}>
          Source :{' '}
          <a
            href="https://geodip.onpe.org/"
            target="_blank"
            rel="noopener noreferrer"
          >
            Observatoire de la précarité énergétique (ONPE)
          </a>
          , GEODIP, 2022 (consultée en décembre 2024)
          <br></br>Export indisponible : cette donnée est diffusée sur demande
          aux territoires par Geodip
        </Body>
        {confortThermique.length > 0 && precariteLogTerritoire ? (
