...
const MapEtatCoursDeau = lazy(() =>
  import('@/components/maps/mapEtatCoursDeau').then((m) => ({
    default: m.MapEtatCoursDeau
  }))
);
...
export const EtatEcoCoursDeau = (props: {
  ...
  const { etatCoursDeau, communesCodes, boundingBox, qualiteEauxBaignade } =
    props;
  ...
  const [exportDataCoursDeau, setExportDataCoursDeau] = useState<
    DataToExport[]
  >([]);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (coursDeau && coursDeau.length > 0) {
            const transformedData = IndicatorExportTransformations.ressourcesEau
              .EtatCoursEau(coursDeau)
              .sort((a, b) =>
                a.libelle_geographique.localeCompare(b.libelle_geographique)
              );
  ...
  const exportData = [
    ...
    },
    {
      ...
      data: IndicatorExportTransformations.ressourcesEau.QualiteSitesBaignade(
        qualiteEauxBaignade
      )
    }
  ];
    ...
    <>
            ...
            <Suspense fallback={<Loader />}>
              ...
              <div className={styles.legendCoursDeauWrapper}>
                <div className={styles.legendCoursDeau}>
                  <Body weight="bold" style={{ alignItems: 'center' }}>
      ...
      <div className={styles.sourcesExportMapWrapper}>
        <Body size="sm" style={{ color: 'var(--gris-dark)' }}>
          Source : Agences de l’eau, 2024 (consultée en février 2026) et
          Ministère des Solidarités et de la Santé, 2025 (consultée en septembre
          2026)
        </Body>
