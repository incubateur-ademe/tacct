...
import {
  arboviroseMapAutochtonesLegend,
  arboviroseMapMoustiqueTigreLegend
...
export const Arbovirose = (props: { arbovirose: ArboviroseModel[] }) => {
  ...
  const aggregatedArbovirose = Object.values(
    arbovirose.reduce<
      Record<
        string,
        { annee: string; nb_cas_importes: number; nb_cas_autochtones: number }
      >
    >((acc, item) => {
      if (!acc[item.annee]) {
        acc[item.annee] = {
  ...
  const casParDepartement = arbovirose
    ...
    .reduce<Record<string, number>>((acc, item) => {
      acc[item.departement] =
        (acc[item.departement] ?? 0) + item.nb_cas_autochtones;
      return acc;
    }, {});

  const exportData =
    IndicatorExportTransformations.sante.Arbovirose(arbovirose);
      ...
      <div className={styles.datavizDoubleMapContainer}>
        <div className={styles.chiffresWrapper}>
          <div className={styles.chiffreDynamiqueWrapper}>
            ...
            <Body>
              Le{' '}
              <DefinitionTooltip title={moustiqueTigre}>
                moustique tigre
              </DefinitionTooltip>{' '}
              est désormais implanté dans 81 départements métropolitains (au 1er
              janv. 2025).
            </Body>
          </div>
          <CustomTooltipNouveauParcours
            ...
            <>
              ...
              <div className={styles.doubleMaps}>
                <div className={styles.singleMaps}>
                  ...
                  >
                    <LegendCompColor
                      legends={arboviroseMapMoustiqueTigreLegend}
                ...
                <div className={styles.singleMaps}>
                  <Body size="sm" style={{ textAlign: 'center' }}>
                    Cas autochtones de dengue, de chikungunya et de zika
                    (cumulés)
                  </Body>
                  <div style={{ position: 'relative' }}>
                    ...
                    >
                      <LegendCompColor
                        legends={arboviroseMapAutochtonesLegend}
                        style={{ gap: '0.5rem 1rem' }}
                      />
                    </div>
                  </div>
      ...
      >
        ...
        >
          <Body size="sm" style={{ color: 'var(--gris-dark)' }}>
            Source carte de présence du moustique tigre : Ministère de la Santé,
            2025 (consulté en janvier 2026)
            <br></br>
            Source carte et graphique de cas autochtones : Santé Publique
            France, 2026 (consultée en février 2026)
          </Body>
          <ExportButton
            data={exportData}
            baseName="arbovirose"
